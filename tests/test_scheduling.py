...
from courtbot.scheduling import fire_at_for_launchd, plan_next_firings, render_racer_plist
...
def test_render_racer_plist_substitutes_fields() -> None:
    ...
    out = render_racer_plist(
        f,
        courtbot_bin="/usr/local/bin/courtbot",
        config_path="/cfg.yaml",
        log_dir="/var/log/courtbot",
        fire_at_local=datetime(2026, 5, 1, 6, 59),
    )
    assert "ai.zipline.courtbot.racer.13234" in out
    ...
    assert "/usr/local/bin/courtbot" in out
