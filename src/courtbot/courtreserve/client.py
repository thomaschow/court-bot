...
from courtbot.courtreserve.parsing import SlotView, parse_confirmation, parse_read_expanded
...
class CourtReserveClient:
    ...
    async def read_consolidated(self, *, day: ddate) -> list[SlotView]:
        ...
        from courtbot.courtreserve.parsing import parse_read_consolidated as _parse
    ...
    async def create_reservation(
        ...
        *,
        ...
        `facility` is the courtbot Facility model. Required for the modal fetch (it has
        org_id + s_id). The booking POST replays every hidden field from the modal so
        per-org / per-modal MVC fields don't break us.
        """
        from courtbot.courtreserve.modal import fetch_modal
