...
from courtbot import logging as log_setup
...
def login(
    ...
    from courtbot.auth.playwright_login import login as do_login
...
def discover(
    ...
    from courtbot.discover.probe import discover as do_discover
...
def book(
    ...
    from courtbot.booking.service import book as do_book
    from courtbot.courtreserve.payloads import BookingCandidate

    cfg = _load(config)
    f = cfg.facility(facility)
    if f.member_id is None:
        raise typer.Exit("facility.member_id is null — run `courtbot discover` first")
...
def schedule_wake(
    ...
    from courtbot.scheduling import (
        fire_at_for_launchd,
    ...
    bin_path = (Path(__file__).resolve().parents[3] / ".venv" / "bin" / "courtbot")
    if not bin_path.exists():
        bin_path = Path("courtbot")
...
def watch(
    ...
    from courtbot.watcher.poller import run_watcher
...
def web(
    ...
    from courtbot.web.app import serve
