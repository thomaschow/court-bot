...
from bay_area_courtbot.auth.session import SessionState, build_client, hydrate
from bay_area_courtbot.config import Config, Facility
    ...
    *,
    ...
    if dry_run:
        ...
        if cfg.notifications.on_dry_run:
            notify_macos("bay_area_courtbot dry-run", f"{facility.id} {cand.date} {cand.start}")
    ...
    try:
        ...
        if cfg.notifications.on_success:
            notify_macos("bay_area_courtbot booked", f"{facility.id} {cand.date} {cand.start} #{confirmation}")
        ...
        if cfg.notifications.on_failure and not isinstance(exc, SlotTaken):
            notify_macos("bay_area_courtbot failed", f"{facility.id} {cand.date} {cand.start}: {exc}")
        return BookResult(
            candidate=cand,
            confirmation_id=None,
            status="failed",
